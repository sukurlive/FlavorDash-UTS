...
async function setupDatabase() {
  try {
    await promiseDb.query(
      `CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || "flavordash_db"}`,
    );
    ...
    const [existingCategories] = await promiseDb.query(
    ...
    const [existingProducts] = await promiseDb.query(
    ...
    if (existingProducts.length === 0) {
      const products = [
        [
          ...
          35000,
          "nasi-goreng.jpg",
          "Nasi goreng dengan telur mata sapi, ayam suwir",
          20,
          1,
        ],
        [
          ...
          25000,
          "mie-ayam.jpg",
          "Mie ayam dengan topping jamur, pangsit",
          15,
          1,
        ],
        [
        ...
        [
          ...
          45000,
          "rendang.jpg",
          "Daging sapi dimasak dengan bumbu rendang",
          8,
          1,
        ],
        [
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    const [newUser] = await promiseDb.query(
      "SELECT id, uuid, name, email, role FROM users WHERE email = ?",
      [email],
    );
    ...
    await promiseDb.query(
      ...
      [newUser[0].id, refreshToken, expiresAt],
    );

    res
      .status(201)
      .json({ success: true, user: newUser[0], accessToken, refreshToken });
  } catch (err) {
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    const [users] = await promiseDb.query(
      "SELECT id, uuid, name, email, password, role FROM users WHERE email = ?",
      [email],
    );
    if (users.length === 0)
...
app.post("/api/addresses", verifyAccessToken, async (req, res) => {
  const userId = req.user.userId;
  const { label, recipient_name, phone, address, city, postal_code, is_default, latitude, longitude } = req.body;
  ...
  try {
    ...
    const uuid = generateUUID();
  await connection.query(
    `INSERT INTO user_addresses (uuid, user_id, label, recipient_name, phone, address, city, postal_code, is_default, latitude, longitude) 
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [uuid, userId, label || "Rumah", recipient_name, phone, address, city, postal_code || null, is_default || false, latitude || null, longitude || null]
  );
  ...
  try {
    ...
    await connection.query(
      `UPDATE user_addresses 
             SET label = ?, recipient_name = ?, phone = ?, address = ?, city = ?, postal_code = ?, is_default = ?, latitude = ?, longitude = ?
...
app.post("/api/cart/add", verifyAccessToken, async (req, res) => {
  ...
  const [products] = await promiseDb.query(
    "SELECT id, stock FROM products WHERE id = ? AND is_available = TRUE",
    [productId],
  );
  if (products.length === 0)
...
app.post("/api/checkout", verifyAccessToken, async (req, res) => {
  const userId = req.user.userId;
  const { paymentMethod = "cash", notes = "", address_id } = req.body;
  const cart = tempCarts.get(userId) || [];
  if (cart.length === 0)
  ...
  try {
    ...
    await connection.query(
      "INSERT INTO orders (uuid, user_id, order_number, total_amount, payment_method, notes, status, address_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
      [
        orderUuid,
        userId,
        orderNumber,
    ...
    for (const item of orderItems) {
      await connection.query(
        "INSERT INTO order_items (uuid, order_id, product_id, quantity, price, subtotal) VALUES (?, ?, ?, ?, ?, ?)",
        [
          generateUUID(),
          orderId,
          item.productId,
    ...
    res.json({
      ...
      order: { orderNumber, totalAmount, status: "pending" },
    });
  } catch (err) {
    await connection.rollback();
...
// UPLOAD BUKTI PENERIMAAN PESANAN
app.post("/api/orders/:id/upload-proof", verifyAccessToken, upload.single("proof_image"), async (req, res) => {
 const { id } = req.params;
 const userId = req.user.userId;

 try {
   // 1. Cek apakah pesanan milik user ini
   const [orders] = await promiseDb.query(
   ...
   // 2. Cek apakah file berhasil diupload
   if (!req.file) {
     return res.status(400).json({ error: "File bukti tidak ditemukan" });
   }

   // 3. Simpan path gambar ke database & ubah status jadi 'completed'
   const imageUrl = `/uploads/${req.file.filename}`;
   await promiseDb.query(
     "UPDATE orders SET proof_image = ?, status = 'completed' WHERE id = ?",
     [imageUrl, id]
   );
   ...
   res.status(500).json({ error: "Gagal mengupload bukti penerimaan" });
 }
});
...
app.get("/api/admin/orders/:id", verifyAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    // ✅ LEFT JOIN untuk mengambil data alamat termasuk latitude & longitude
    const [orders] = await promiseDb.query(
      `SELECT o.*, 
              u.name as user_name, u.email as user_email,
              ua.recipient_name, ua.phone, ua.address, ua.city, ua.latitude, ua.longitude
       FROM orders o 
       JOIN users u ON o.user_id = u.id 
...
// UPDATE data user (Admin only) - untuk fitur Edit
app.put("/api/admin/users/:id", verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    if (email && email !== existingUsers[0].email) {
      const [emailCheck] = await promiseDb.query(
...
// DELETE user (Admin only) - untuk fitur Hapus
app.delete("/api/admin/users/:id", verifyAdmin, async (req, res) => {
  const { id } = req.params;
  const adminId = req.user.userId; // ID admin yang sedang login

  try {
    // PROTEKSI: Cegah admin menghapus dirinya sendiri
    ...
    // Hapus user (CASCADE akan otomatis hapus data terkait)

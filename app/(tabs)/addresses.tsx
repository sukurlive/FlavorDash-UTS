...
// ✅ TAMBAHKAN latitude & longitude KE TYPE
type Address = {
  id: number;
  label: string;
...
export default function AddressesScreen() {
  ...
  // ✅ TAMBAHKAN latitude & longitude KE FORM DATA
  ...
  // ✅ FUNGSI BARU: AMBIL KOORDINAT GPS
  const fetchCurrentLocation = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Izin Ditolak",
          "Aplikasi memerlukan izin lokasi untuk mengambil koordinat.",
        );
        setFetchingLocation(false);
        return;
      }

      Alert.alert("Mencari Lokasi", "Sedang mengambil koordinat GPS...");
      ...
      Alert.alert("Berhasil", "Lokasi berhasil diambil dan disimpan!");
    } catch (error) {
      console.error("Location error:", error);
      Alert.alert(
        "Error",
        "Gagal mengambil lokasi. Pastikan GPS perangkat Anda aktif.",
      );
    } finally {
      setFetchingLocation(false);
    }
    ...
    try {
      if (editingAddress) {
        await axios.put(
          ...
          formData, // ✅ formData sekarang sudah mencakup latitude & longitude
          { headers: { Authorization: `Bearer ${token}` } },
        );
      ...
      Alert.alert(
        "Error",
        error.response?.data?.error || "Gagal menyimpan alamat",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const setDefault = async (address: Address) => {
    ...
    try {
      await axios.put(
        `${API_URL}/api/addresses/${address.id}/default`,
        {},
        { headers: { Authorization: `Bearer ${token}` } },
      );
  ...
  const openEditModal = (address: Address) => {
    ...
    setFormData({
      ...
      latitude: address.latitude || null, // ✅ ISI KOORDINAT LAMA
      longitude: address.longitude || null, // ✅ ISI KOORDINAT LAMA
    });
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              {formData.latitude && formData.longitude && (
                <Text style={styles.coordsText}>
                  Koordinat tersimpan: {formData.latitude.toFixed(5)},{" "}
                ...
                <View style={styles.labelButtons}>
                  {["Rumah", "Kantor", "Lainnya"].map((l) => (
                    <TouchableOpacity
                      ...
                      style={[
                        styles.labelButton,
                        formData.label === l && styles.labelButtonActive,
                      ]}
                    ...
                    >
                      <Text
                        style={[
                          ...
                          formData.label === l && styles.labelButtonTextActive,
                        ]}
                      >
                        {l}
                      </Text>
...
const styles = StyleSheet.create({
  ...
  // ✅ STYLE BARU UNTUK TOMBOL LOKASI
  locationButton: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
